...
Flare has no transition tiles between its ground types, so the renderer softens floor boundaries itself: where grass meets a road, mud, paving or water, the higher material (paving, then grass, dirt, mud, water) spills over the lower one's edge through a noisy alpha mask (rendered once per cell into a small canvas, a few cells per frame; narrow strips and lone cells keep a patch of their own material), and floor variants are picked with low-frequency noise over the variants sorted by brightness, with cobbles, stones and cracked slabs sprinkled in sparsely.
